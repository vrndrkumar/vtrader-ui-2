...
interface Persisted { layoutId: string; activePanelId: string; panels: Record<string, PanelConfig>; sync: SyncState }
function load(): Persisted {
  const base: Persisted = { layoutId: '2h', activePanelId: 'p0', panels: defaultPanels(), sync: DEFAULT_SYNC }
  try {
...
export const useChartLayoutStore = create<LayoutStore>((set, get) => {
  const commit = (partial: Partial<LayoutStore>) => {
    set(partial)
    const s = get()
    try { localStorage.setItem(LS_KEY, JSON.stringify({ layoutId: s.layoutId, activePanelId: s.activePanelId, panels: s.panels, sync: s.sync })) } catch { /* ignore */ }
  }
  return {
    ...initial,
    setLayout: (layoutId) => {
